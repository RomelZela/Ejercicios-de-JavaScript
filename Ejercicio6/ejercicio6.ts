...
const roundTo = (num: number, decimals: number) :number => {
    ...
    let getDecimals = num.toString().split('.');
    
    // Modificamos la longitud de los decimales.
    let setLengthDecimals = getDecimals[1].substring(0, decimals)
    ...
    */
    let lastNumDecimal = getDecimals[1].charAt(setLengthDecimals.length);
    console.log('lastNumDecimal: ' +lastNumDecimal)
    
    // Parseamos a integer para poder sumar uno y hacer el redondeo adecuado.
    let decimalToInt = parseInt(lastNumDecimal)
    
    // Operador ternario, si el número es mayor a 5 y menor a 8, le sumamos 1, de lo contrario queda igual.
    let roundDecimal = decimalToInt >= 5 && decimalToInt <= 9 ? parseInt(setLengthDecimals)+1 : parseInt(setLengthDecimals);
    ...
    let concatNumDecimals = getDecimals[0].concat('.'+ roundDecimal);
    
    // Parseamos para tener los números y trabajar con los valores adecuados.
    let getDecimmalsToInt = parseFloat(concatNumDecimals)
